...
def main():
    ...
    if target_cog2seqid.shape[0] == 0:
        print("No target proteins found in the COGs, please check your input.")
        return
    ...
    for target in targets:
        ...
        for index, row in cog_meme.iterrows():
            ...
            png_path1,png_path2,png_path3,png_path4 = row.get("png_path_1"), row.get("png_path_2"), row.get("png_path_3"), row.get("png_path_4")
            tsv_path1,tsv_path2,tsv_path3,tsv_path4 = row.get("tsv_path_1"), row.get("tsv_path_2"), row.get("tsv_path_3"), row.get("tsv_path_4")
    ...
    for fasta in fastaList:
        ...
        for record in SeqIO.parse(fasta, "fasta"):
            ...
            with open(f"{tf_data}/tf_cog2seqid_location.csv", mode = 'a') as out:
                out.write(f"{cog};{record.id};{start};{end};{strand}\n")
...
def get_upstream_lengths(tf_data, cog):
    ...
    try:
        ...
        print(f"Warning: {ups100}, {ups300} or {ups500} not found, skipping.")
...
def get_cog2png_df(tf_data, meme_info, cogs):
    ...
    for cog in cogs:
        tf_fam = cog.split("_")[0]
        print(f"Processing COG {cog} in TF family {tf_fam}...")
                ...
                for motiflength in [10, 20, 30]:
                    for nmotifs in [1,2,3,4]:
                        ...
                        if os.path.exists(png_path) and os.path.exists(tsv_path):
                            rows.append({
                                "cog": cog, "ups": ups, "mode": mode,
                                "motiflength": motiflength, "nmotifs": nmotifs,
                                "png_path": png_path, "tsv_path": tsv_path,
