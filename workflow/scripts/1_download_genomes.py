...
def download_assembly_summary(databases, repository, date):
    for db in databases:
        ...
        if not os.path.exists(f"assembly_summary_{db}_{month}.txt"):
            ...
            os.system(f"wget -q https://ftp.ncbi.nlm.nih.gov/genomes/{db}/{repository}/assembly_summary.txt -O assembly_summary_{db}_{month}.txt")
        else:
...
def download_gcf(ftp_list, overwrite, n_genomes):
    for i in range(n_genomes):
        ...
        url = f"https://{ftp_list[i]}"
        
        if overwrite == "True":
            print(f"Downloading assembly {gcf}")
            os.system(f"wget -r -np -nH --cut-dirs=7 --reject='index.html*' --execute robots=off -q {url}/ -P ./{gcf}/")
        else:
            if os.path.exists(gcf):
                print(f"Directory for assembly {gcf} already exists, skipping...")
                continue
            else:
                print(f"Downloading assembly {gcf}")
                os.system(f"wget -r -np -nH --cut-dirs=7 --reject='index.html*' --execute robots=off -q {url}/ -P ./{gcf}/")
