...
rule run_dbcan:
    ...
    shell:
        ...
        run_dbcan easy_substrate --mode protein \
            --output_dir {outdir}/dbcan/{wildcards.strain} \
...
rule run_proteinortho:
    ...
    shell:
        ...
        "mv {project_name}.blast-graph_clean {outdir}/proteinortho/{project_name}.blast-graph_clean; mv {project_name}.proteinortho.tsv.xml {outdir}/proteinortho/{project_name}.proteinortho.tsv.xml"
...
rule classify_ipr_tfs:
    ...
    benchmark:
        ...
        #repeat("benchmarks/somecommand/{sample}.tsv", 3)
    shell:
        "python workflow/scripts/5_classify_tfs_IPR.py --ipr_dir {params.ipr_dir} --tf_domains resources/tf_ipr_domains.tsv --proteinortho_tsv {input.proteinortho_tsv} --outdir {outdir}/tf_data/ --tf_families {params.tf_families} --perc_threshold {params.perc_threshold}"
...
rule rename_orthogroup_fastas:
    ...
    shell:
        "rename -f 's/.proteinortho.tsv./_/g' {outdir}/tf_data/*/cogs/*fasta"
...
rule integrate_dbcan_tf:
    ...
    shell:
        ...
        "--dbcan_dir {outdir}/dbcan/ "
        "--gff_db_dir {outdir}/cds_gff/ "
        "--db_dir resources/dbcan/db/ "
        "--tf_classification {outdir}/tf_data/TF_classification.tsv "
        "--method {params.method} "
        "--extension {params.extension} "
        "--neighbours {params.neighbours} "
        "--outdir {outdir}/dbcan_tf/"
...
#         "workflow/envs/aurtho.yml"
#     shell:
#         "python workflow/scripts/combine_benchmarks.py --benchmark_dir {outdir}/benchmarks/ --output_file {output.combined}"
